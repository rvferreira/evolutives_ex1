...
def crossover(parents_vector, pc, crossover_method):
	...
	if crossover_method == 'MEAN':
		for i in range(len(parents_vector[0])):
			parents_merge_vector.append(round(fsum([parents_vector[0][i], parents_vector[1][i]])/len(parents_vector), DECIMAL_APPROX))
		...
		for j in range(len(population_vector[i])):
			...
			if chance < mutation_rate:
				if mutation_mode=='PLUS_MINUS':
					if randrange(0,2)==1:
						population_vector[i][j]=round(population_vector[i][j]+mutation_intensity, DECIMAL_APPROX)
					else:
						population_vector[i][j]=round(population_vector[i][j]-mutation_intensity, DECIMAL_APPROX)
...
def ackley_fitness(population_vector, index, individual_dimensions_count):
	...
	return round(-C1*exp(-C2*sqrt((1.0/individual_dimensions_count)*s1)-exp((1.0/individual_dimensions_count)*s2))+C1+1, DECIMAL_APPROX)
